...
export default function PricingManagement() {
  ...
  const fetchPricingData = async () => {
    try {
      ...
      // Handle API response format - extract data or use empty array as fallback
      setProfiles(Array.isArray(profilesData) ? profilesData : (profilesData.data || []))
      setTnvsFares(Array.isArray(tnvsData) ? tnvsData : (tnvsData.data || []))
      setTaxiFares(Array.isArray(taxiData) ? taxiData : (taxiData.data || []))
      setTolls(Array.isArray(tollsData) ? tollsData : (tollsData.data || []))
    } catch (err) {
